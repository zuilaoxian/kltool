...
if rs.eof then call kltool_err_msg("错误的vip")
jin=clng(rs("jinbi"))
yan=clng(rs("jinyan"))
xian=clng(rs("xian"))
rs.close
...
if xian<>1 then call kltool_err_msg("请不要钻空子")
if my="" then call kltool_err_msg("请填写开通月份")
if not Isnumeric(my) then call kltool_err_msg("必须是数字")
if clng(money)<(jin*my) then call kltool_err_msg("金币不足,需"&jin*my)
if clng(expr)<(yan*my) then call kltool_err_msg("经验不足,需"&yan*my)
if clng(SessionTimeout)<>clng(id) and clng(SessionTimeout)>0 then call kltool_err_msg("当前有vip，请先解除")
