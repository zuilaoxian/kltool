...
wid=request("wid")
page=request("page")
if wid=1 then call kltool_err_msg("无法删除")
set rs=server.CreateObject("adodb.recordset")
...
wid=request("wid")
page=request("page")
if wid=1 then call kltool_err_msg("无法修改")
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from [DomainName] where id="&wid,conn,1,1
If rs.eof Then call kerr("无此记录")
%>
...
hid=request("hid")
page=request("page")
if ym="" or hid="" then call kltool_err_msg("域名或转向不能为空")
if wid=1 then call kltool_err_msg("无法修改")
...
If Not rs.eof Then call kltool_err_msg("此域名已存在")
rs.close
set rs=nothing
