...
if not Isnumeric(z) or z<1 then call kltool_err_msg("验证时长必须为数字且不能小于1分钟")
if not Isnumeric(listsize) or not Isnumeric(listsize2) then call kltool_err_msg("列表数量必须为数字")

if listsize="" or listsize<0 then call kltool_err_msg("不能为空或小于0")
if listsize2="" or listsize2<0 then call kltool_err_msg("不能为空或小于0")
set rs=server.CreateObject("adodb.recordset")
...
page=request("page")
lx=request("lx")
if kltool_admin_log_del<>1 then call kltool_err_msg("不允许删除操作日志\n请在"&kltool_path&"inc/config.asp中配置")
