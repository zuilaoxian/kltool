...
clid=request("clid")
if (action="3" or action="4") and click="" then call kltool_err_msg("点击量不能为空")
if click<>"" and not Isnumeric(click) then call kltool_err_msg("点击量必须为数字")
if tid="" then call kltool_err_msg("未选择任何记录")
if action="1" then
...
if rearrtid(d)<>"" then
	for i=1 to click
		...
		if recontent="" then call kltool_err_msg("没有查询到回复语！")
			reuser=kltool_bbs_reuser
		if reuser="" then call kltool_err_msg("没有查询到用户！")
...
if tid="" then call kltool_err_msg("id不能为空")
if not Isnumeric(tid) then call kltool_err_msg("必须为数字")
set rs=Server.CreateObject("ADODB.Recordset")
...
if tid="" or not Isnumeric(tid) then call kltool_err_msg("不能为空且必须为数字")
if pub="" or not Isnumeric(pub) then call kltool_err_msg("不能为空且必须为数字")
set rs=Server.CreateObject("ADODB.Recordset")
rs.open"select * from [wap_bbs] where id="&tid,conn,1,2
if rs.bof or rs.eof then call kltool_err_msg("无此记录")
