...
elseif pg="shoping" then
	...
	If rs.eof Then call kltool_msge("错误，不存在此记录")
	lx=clng(rs("lx"))
	jiage=clng(rs("jiage"))
	rs.close
	set rs=nothing
	...
	if jtzsl>=myzsl then call kltool_msge("对不起，您今日购买的CDK总数量达到上限！")
	rs.close
	set rs=nothing
	'分类量
	set rs=server.CreateObject("adodb.recordset")
	...
	set rs=nothing
		...
		rs.close
		set rs=nothing
	if flsl>=mysl then call kltool_msge("对不起，您今日购买的本类CDK总数量达到上限！")
