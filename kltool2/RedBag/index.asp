...
Response.Write kltool_head("互动红包",0)
...
elseif pg="yes" then
	...
	'-----各种判断
	if hblx="" or (not Isnumeric(hblx)) then call kltool_msge("请选择红包类型")
	if hbsl="" or (not Isnumeric(hbsl)) or hbsl<1 then call kltool_msge("红包个数不能为空\n大于1\n且必须是数字")
	if moneysl="" or (not Isnumeric(moneysl)) or moneysl<1 then call kltool_msge(""&sitemoneyname&"数量不能为空\n大于1\n且必须是数字")
	if clng(moneysl)>clng(money) then call kltool_msge("您的"&sitemoneyname&"不够哦")
	if hbsl>clng(kltool_hb_sx(1)) then call kltool_msge("超出红包个数上限")
	if moneysl>clng(kltool_hb_sx(2)) then call kltool_msge("超出红包金额上限")
	if hblx=1 then
		if clng(hbsl*moneysl)>clng(money) then call kltool_msge("您的"&sitemoneyname&"不够")
	elseif hblx=2 then
		if int(moneysl/hbsl)<1 then call kltool_msge("分配"&sitemoneyname&"失败\n因为"&sitemoneyname&"过少")
	elseif hblx=3 then
		if hbay="" or len(hbay)>16 then call kltool_msge("暗语不能为空\n且不能大于16字符")
	end if
	'-----写入红包1
		set rs=server.CreateObject("adodb.recordset")
...
elseif pg="yes3" then
	...
	if kltool_hb_last(yes3_hbly)<=0 then call kltool_msg("来迟一步，毛都不剩")
	if kltool_hb_yes(yes3_hbly)>=1 then call kltool_msg("贪心不足蛇吞象")
	if yes3_hbay<>"" and yes3_hbay<>kltool_hb_ay(yes3_hbly) then call kltool_msg("暗语不正确哟")

	sql="select top 1 * from [kltool_hb_log] where hb_ly='"&yes3_hbly&"' and hb_userid=0 order by newid()"
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,2
	if rs.bof or rs.eof then call kltool_msg("来迟一步，毛都不剩")
	moneysl=rs("hb_money")
	rs("hb_userid")=userid
	rs("hb_date")=now()
	rs.update
